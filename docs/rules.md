...
Type hints obligatoires sur toutes les fonctions et méthodes publiques. `mypy --strict` passe sans erreur avant tout commit.

### Commentaires

- Le commentaire explique le POURQUOI, jamais le QUOI
- Pas de commentaire qui paraphrase le code (`# increment i` sur `i += 1`)
- Docstring obligatoire sur toute fonction ou classe publique — décrire le contrat, pas l'implémentation
...
- Tout modèle hérite de `BaseModel` (UUID PK, `created_at`, `updated_at`)
- Les modèles fédérables héritent de `ActivityPubMixin` (`ap_id`, `inbox`, `outbox`, `local`)
- `ap_id` et `local` ont toujours `db_index=True`
- `status` est indexé sur Character, Report, Follow, LinkRequest
- Les requêtes combinées fréquentes ont un index composite
- Le contenu fédéré n'est jamais supprimé — `deleted_at` + `ActiveManager`
...
- Maximum 10 requêtes SQL par page — vérifier avec `django-debug-toolbar` en dev
- Toute liste est paginée — jamais de queryset non bornée dans une vue
- `select_related` / `prefetch_related` obligatoires sur toute queryset paginée
