...
- [x] Les 3 profils locaux (User/Character/Game) exposent un bouton follow ; les mutations sont en POST (`@require_POST`).
- [x] Suivre un Character/Game distant Suddenly crée un `Follow` polymorphe ; un acteur Mastodon reste Person-only (pas de 500).
...
- [x] `GET {user actor}/following` renvoie un `OrderedCollection` conforme (pas de 404) ; `totalItems` = total réel.
- [x] Les compteurs followers/following du profil sont cliquables et ouvrent une liste HTML paginée.
- [x] `assertNumQueries` borne les listes (pas de N+1 sur `follower`).
...
- [x] Le profil d'un compte distant Suddenly affiche parties/personnages/activité disponibles.
- [x] Le profil d'un compte Mastodon affiche résumé + activité récente sans section Suddenly ni erreur.
- [x] Un fetch distant en échec dégrade en profil minimal (pas de 500).
...
🤖 2026-07-19 — Phase 2, critère 2 : `follow_toggle` (`characters/follow_views.py`) passe à `@require_POST` (avant `@login_required`), le garde-méthode manuel (`HttpResponseNotAllowed`) retiré — conforme `03-htmx-patterns.md`. La vue restant générique (`actor_model_for`), ce seul changement couvre les 3 acteurs locaux (User/Character/Game). Vérifié par un smoke test pytest temporaire (GET → 405 ; POST toggle Character et Game), supprimé après validation — couverture permanente consolidée dans `tests/activitypub/test_follow_ui.py` (Phase 5).

🤖 2026-07-19 — Phase 2 complète (critère 3, DEC-C4) : `_resolve_remote_actor_type(ap_id, actor_data)` (`activitypub/federation_views.py`) classifie un acteur distant fetché — `type: "Group"` + instance Suddenly connue (`FederatedServer.is_suddenly_instance()`) → `"game"` ; `type: "Person"` + clé `"status"` (émise sans condition par `serialize_character`, namespace `suddenly:status`) + instance Suddenly → `"character"` ; tout le reste (y compris un acteur `"status"` venant d'une instance non-Suddenly ou inconnue) → `"user"`. `remote_follow_toggle` route vers `get_or_create_remote_character`/`get_or_create_remote_game`/`get_or_create_remote_user` selon ce type et crée le `Follow` polymorphe correspondant (`content_type_for_actor`). Fix mypy associé dans `tasks.py::send_undo_follow_activity` : `target_model: Any = actor_model_for(target_type)` (l'absence d'annotation faisait perdre le typage des champs spécifiques du modèle résolu dynamiquement, `attr-defined` sur `.inbox_url`). Task 5 : `components/remote_follow_button.html` reçoit désormais `target_type` en contexte (`federation_views.py` — `remote_follow_toggle` et `remote_profile`, ce dernier le calcule sans appel réseau supplémentaire puisque `actor_data` est déjà fetché) et le porte via `data-target-type="{{ target_type }}"` sur le conteneur + un `<input type="hidden" name="target_type">` dans chaque formulaire — non lu par la vue (qui reclassifie server-side à chaque toggle, `ap_id`-driven, jamais fait confiance au POST), mais aligné structurellement sur `components/follow_button.html` (mêmes classes `btn-primary`/`btn-secondary btn-sm`, mêmes icônes `i-lucide-check`/`i-lucide-user-plus`, même prop `target_type` explicite). Pas d'injection JS — autoescape HTML standard suffit, `|escapejs` non requis (aucun contexte `<script>`/attribut `on*`). Vérifié par deux smoke tests pytest temporaires : (1) 3 scénarios DEC-C4 (Character Suddenly, Game Suddenly, Person Mastodon avec `"status"` piégeur → reste `"user"`, jamais 500) via `httpx.Client.post` mocké (seul point d'interception fiable sous Celery eager — le mock de `sign_and_deliver` et de `.delay` directement n'interceptait pas la livraison réelle) ; (2) rendu HTML du bouton confirmant `data-target-type="user"` et le hidden input. Les deux fichiers supprimés après validation — couverture permanente consolidée dans `tests/activitypub/test_follow_ui.py` (Phase 5). `ruff`/`mypy` clean sur `inbox.py`, `tasks.py`, `federation_views.py`.

🤖 2026-07-19 — Phase 3, critère 1 : `user_following` ajouté (`activitypub/views.py`), aligné sur l'IRI `{user actor}/following` annoncée par `serialize_user` (`prefetch_related("target")` sur le GFK polymorphe `Follow.target`, batché par content type — pas de N+1). `game_followers`/`character_followers` ajoutés en complément pour fermer le 404 identifié au point 5 des « Manques identifiés » — `game_following`/`character_following` volontairement **non** ajoutés : `serialize_game`/`serialize_character` n'annoncent aucune IRI `following` (vérifié dans `serializers.py`), les ajouter aurait été hors périmètre. Routes dans `activitypub/urls.py` — non namespacées (`suddenly/activitypub/urls.py` n'a pas d'`app_name` et est inclus sans `namespace=` dans `suddenly/urls.py`) : `reverse("user-following", ...)`, pas `reverse("activitypub:user-following", ...)`, malgré la formulation informelle du plan. Vérifié par un smoke test pytest temporaire (4 tests : `user_following` polymorphe User+Character, `game_followers`, `character_followers`, 404 user inconnu), supprimé après validation. `ruff check`/`mypy suddenly/activitypub/` clean.

🤖 2026-07-19 — Phase 4 complète (les 3 critères) : nouveau module `activitypub/follow_federation.py` — `fetch_remote_actor_collections(actor_data, *, is_suddenly)` fetch la 1re page de l'outbox de l'acteur distant (`fetch_ap_json`, SSRF-safe) et en dérive `activity` (tout acteur — unwrap `Create` une fois, garde `Article`/`Note`), et pour un acteur Suddenly uniquement `games` (déduplication des IRIs `context`, un fetch borné par jeu via `fetch_ap_actor`, `MAX_GAMES=5`) et `characters` (déduplication des `tag` Mention `href`/`name`, aucun fetch supplémentaire — le nom est déjà porté par `serialize_report`, `MAX_CHARACTERS=5`) ; `MAX_OUTBOX_ITEMS=10` borne le fetch lui-même, aucune pagination distante suivie au-delà de la 1re page (risk register). Toute fonction dégrade en liste vide sur réponse absente/malformée — jamais d'exception (DEC-C5). `federation_views.remote_profile` calcule `is_suddenly` une seule fois (nouveau helper `_is_suddenly_actor`, extrait de `_resolve_remote_actor_type` pour éviter une requête `FederatedServer` dupliquée — `_resolve_remote_actor_type` accepte maintenant `is_suddenly` en kwarg optionnel, `remote_follow_toggle` inchangé) et enrobe l'appel à `fetch_remote_actor_collections` dans un `try/except Exception` (défense en profondeur au-dessus des fetchs déjà tolérants) pour garantir qu'un échec inattendu ne remonte jamais en 500. Nouveau template `templates/federation/_remote_profile_sections.html` (inclus dans `remote_profile.html`) : sections « Games »/« Characters » affichées seulement si `is_suddenly`, section « Recent activity » toujours affichée, état vide explicite (`{% trans %}`) pour chacune — jamais d'erreur visible. Vérifié par 5 tests pytest temporaires (dérivation Suddenly complète, Mastodon activité-seule sans fetch d'acteur superflu, outbox injoignable → collections vides, `outbox` absent du profil → collections vides, exception dans l'enrichissement au niveau vue → 200 avec sections vides), supprimés après validation — couverture permanente consolidée dans `tests/activitypub/test_follow_ui.py` (Phase 5, Task 2 du plan). `ruff check`/`ruff format`/`mypy suddenly/activitypub/` clean ; lint design système (`design/lint/lint-files.mjs`) clean sur les 4 fichiers touchés (pas de composant "chip" existant — réutilisation de `.badge`, déjà utilisée pour un usage similaire dans `_cast_entry.html`).

🤖 2026-07-19 — Phase 3, critères 2+3 (un seul commit pour les deux) : `users/views.py` ajoute `followers_list`/`following_list` (`@require_GET`, `htmx_render` pour partial/full selon `getattr(request,'htmx',False)`, `Paginator(…, 20)`) ; `users/urls.py` ajoute `<username>/followers/` et `<username>/following/` (namespace `users`, donc `{% url 'users:followers' %}`/`{% url 'users:following' %}` — routes réellement servies sous le préfixe `@` défini dans `suddenly/urls.py` ligne 46, soit `/@<username>/followers/`, pas `/users/<username>/followers/` ; à ne pas confondre avec les IRIs AP non namespacées `activitypub/urls.py`). 4 nouveaux templates : `followers_list.html`+`_followers_page.html` (liste simple `follower`), `following_list.html`+`_following_page.html` (liste polymorphe, branchement sur `follow.content_type.model` — `user`/`character`/`game` — pour résoudre l'URL cible, l'icône (`i-lucide-user` User/Character, `i-lucide-book-open` Game — `Game` n'a pas de champ `avatar`, seulement `cover`) et le libellé de type) ; sentinel infinite-scroll conforme (`hx-trigger="revealed"`, absent si `has_next=False`). `profile.html` : compteurs devenus des `<a href="{% url 'users:followers'/'following' %}">`. `followers_list` : `select_related('follower')` (1 query + join, pas de N+1 par follower). `following_list` : `select_related('content_type').prefetch_related('target')` (1 query de base + 1 prefetch par type de content_type effectivement présent — borné par le nombre de types, pas par N). Critère « `assertNumQueries` borne les listes » vérifié par un smoke test pytest temporaire comparant le nombre de requêtes capturées (`CaptureQueriesContext`) entre un petit lot et un lot 3-4× plus gros (après un appel de warm-up pour neutraliser le cache `instance_settings`, dont le premier miss ajoute des requêtes d'écriture non liées à N) — égalité stricte obtenue dans les deux vues, prouvant l'absence de N+1. Supprimé après validation ; couverture permanente consolidée dans `tests/activitypub/test_follow_ui.py` (Phase 5). `ruff check`/`ruff format --check`/`mypy suddenly/users/views.py` clean ; `node design/lint/lint-files.mjs` clean sur les 5 templates touchés/créés. Phase 3 terminée (3/3 critères).
