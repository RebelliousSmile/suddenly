...
class TestHomeToggle:
    def test_anonymous_home_renders_vitrine_with_stats(
        ...
        # Tokens follow the frozen design-system v1.3 contract (semantic-*) and the
        # explicit container-query grid, per the phase-6 merge arbitration.
        assert "from-semantic-surface to-semantic-background" in body  # hero, vitrine-only
        assert "grid-cols-2 @sm:grid-cols-4" in body
    ...
    def test_authenticated_home_renders_feed(self, db: Any, client: Client, user: User) -> None:
        ...
        assert "from-semantic-surface to-semantic-background" not in body
